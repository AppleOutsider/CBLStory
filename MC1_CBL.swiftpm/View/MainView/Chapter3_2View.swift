...
struct Chapter3_2View: View {
        ...
        NavigationView{
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    Image(Chapter3_2Data.dataList[dataindex].bgImage)
                        .resizable()
                        .scaledToFill()
                        .ignoresSafeArea(.all)
                        .frame(width: geo.size.width, height: geo.size.height)
                        .onAppear {
                            Chapter3_2Bgm.instance.playSound()
                        }
                    Rectangle()
                    ...
                    NavigationLink(destination: Chapter4_1View(), tag: 1, selection: self.$tag ) {}
                    
                    VStack {
                        Spacer()
                        Button {
                            
                            if dataindex + 1 == Chapter3_2Data.dataList.count {
                                Chapter3_2Bgm.instance.stopSound()
                                self.tag = 1;
                            } else {
                                dataindex += 1
                                if let soundeffect = Chapter3_2Data.dataList[dataindex].lineModels.soundEffect {
                            ...
                            if Chapter3_2Data.dataList[dataindex].lineModels.names == "내레이션" {
                                CustomBlackCenter(name: Chapter3_2Data.dataList[dataindex].lineModels.names,
                                                  line: text, soundEffect: nil, geo: geo)
                            } else {
                                
                                switch Chapter3_2Data.dataList[dataindex].talkingPersonDirection {
                                case .left:
                                    CustomWhiteLeft(name: Chapter3_2Data.dataList[dataindex].lineModels.names,
                                                    line: text, soundEffect: nil, geo: geo)
                                case .right:
                                    CustomWhiteRight(name: Chapter3_2Data.dataList[dataindex].lineModels.names,
                                                     line: text, soundEffect: nil, geo: geo)
                                default:
                                    CustomWhiteCenter(name: Chapter3_2Data.dataList[dataindex].lineModels.names,
                                                      line: text, soundEffect: nil, geo: geo)
                                }
    ...
    func typeWriter(at position: Int = 0) {
        ...
        if position < Chapter3_2Data.dataList[dataindex].lineModels.lines.count {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                text.append(Chapter3_2Data.dataList[dataindex].lineModels.lines[position])
...
struct Chapter3_2View_Previews: PreviewProvider {
