...
// 베이스로 사용하자
func CustomTextView(lineModels: LineModels) -> some View {
    HStack {
...
// 내레이션 베이스
func CustomtextBlack(lineModels: LineModels) -> some View {
    HStack {
        VStack(alignment: .leading, spacing: 30) {
            
            Text(lineModels.names)
                .bold()
                .textPersonWhite() // .textPersonWhite(geo: GeoProxy)  geo.size.width - 40 geo.size.height / 4 
...
// MARK: - 여기서 4가지 버전으로
// 흰 배경 왼쪽 대사
func CustomWhiteLeft(name: String, line: String, soundEffect: String?) -> some View {
    CustomTextView(lineModels: LineModels(names: name, lines: line, soundEffect: nil))
        .textWhiteLeft()
}

// 흰 배경 오른쪽 대사
func CustomWhiteRight(name: String, line: String, soundEffect: String?) -> some View {
    CustomTextView(lineModels: LineModels(names: name, lines: line, soundEffect: nil))
        .textWhiteRight()
}

// 흰 배경 가운데 대사
func CustomWhiteCenter(name: String, line: String, soundEffect: String?) -> some View {
    CustomTextView(lineModels: LineModels(names: name, lines: line, soundEffect: nil))
        .textWhiteCenter()
}

// 검은 배경 내래이션
func CustomBlackCenter(name: String, line: String, soundEffect: String?) -> some View {
    CustomTextView(lineModels: LineModels(names: name, lines: line, soundEffect: nil))
        .textBlack()
}
...
struct lineTextView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        VStack {
            ...
            //            Spacer()
            CustomWhiteLeft(name: "testname", line: "testline", soundEffect: nil)
        }
        .background(Image("bg"))
    }
}
